# Get API_KEY from this website: https://ipgeolocation.io/  and replace the "0000" with it

$API_KEY = "0000"
$LOG_NAME = "failed_rdp.log"
$LOGFILE_PATH = "C:\ProgramData\$($LOG_NAME)"
...
<# Function to create sample log files that will be used to train the Log Analytics workspace." #>
...
# This will create the log file if it doesn't exist already
...
while ($true)
{
    ...
    # Step through each event collected, get geolocation and add new events to the custom log
    foreach ($event in $events) {

        # $event.properties[19] is the source IP address of the failed logon
        # This if-statement will proceed if the IP address exists
        if ($event.properties[19].Value.Length -ge 4) {
            ...
            if (-Not ($log_contents -match "$($timestamp)") -or ($log_contents.Length -eq 0)) {
                ...
                # Make request to the geolocation API, https://ipgeolocation.io/documentation/ip-geolocation-api.html
